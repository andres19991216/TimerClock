...
void ReadCounter( signed char* read_buffer ){
    ...
    if( counter_direction_flg == C_INC ) {
        read_buffer[ C_HOUR + 1 ]   = counter_up_hour   % 10;
        read_buffer[ C_HOUR ]       = counter_up_hour   / 10;
        read_buffer[ C_MINUTE + 1 ] = counter_up_minute % 10;
        read_buffer[ C_MINUTE ]     = counter_up_minute / 10;
        read_buffer[ C_SECOND + 1 ] = counter_up_second % 10;
        read_buffer[ C_SECOND ]     = counter_up_second / 10;
    }
    
    else if( counter_direction_flg == C_DEC ){
        read_buffer[ C_HOUR + 1 ]   = counter_down_hour   % 10;
        read_buffer[ C_HOUR ]       = counter_down_hour   / 10;
        read_buffer[ C_MINUTE + 1 ] = counter_down_minute % 10;
        read_buffer[ C_MINUTE ]     = counter_down_minute / 10;
        read_buffer[ C_SECOND + 1 ] = counter_down_second % 10;
        read_buffer[ C_SECOND ]     = counter_down_second / 10;
    }
