...
//    printf("Hello");
    ...
    while(1)
    {
        ...
        else { BLINK_TIME = DISP_BLINK_TIME; if( SYSTEM_STATE_FLG == COUNTER_COMP_STATE ) blink_enable_flg = 0; }
...
void LEDProcess() {
    ...
    switch( SYSTEM_STATE_FLG ){
        ...
        case COUNTER_PLAY_STATE:
            display_datas[ 7 ] |= START_STOP_GREEN_LED_ON;
            if( counter_skip_flg ) {
                led_blink_cnt ++;
...
//        unsigned char rtc_eeprom_values[10];
//        rtc_eeprom_values[RTC_SECOND_ADDRESS]  = ReadEeprom(RTC_SECOND_ADDRESS);
//        rtc_eeprom_values[RTC_MINUTE_ADDRESS]  = ReadEeprom(RTC_MINUTE_ADDRESS);
//        rtc_eeprom_values[RTC_HOUR_ADDRESS]    = ReadEeprom(RTC_HOUR_ADDRESS);
//        rtc_eeprom_values[RTC_WEEKDAY_ADDRESS] = ReadEeprom(RTC_WEEKDAY_ADDRESS);
//        rtc_eeprom_values[RTC_DAY_ADDRESS]     = ReadEeprom(RTC_DAY_ADDRESS);
//        rtc_eeprom_values[RTC_MONTH_ADDRESS]   = ReadEeprom(RTC_MONTH_ADDRESS);
//        rtc_eeprom_values[RTC_YEAR_ADDRESS]    = ReadEeprom(RTC_YEAR_ADDRESS);
//        if( rtc_eeprom_values[RTC_YEAR_ADDRESS] == 0 || rtc_eeprom_values[RTC_YEAR_ADDRESS] == 255 ) Ds1307Write( &rtc_default_values );
//        else                                                                                         Ds1307Write( &rtc_eeprom_values );
    }
