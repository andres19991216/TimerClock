...
void main(void) {
    ...
    while(1)
    {
        ...
        if( counter_down_comp_flg ) {
            BLINK_TIME = 200; blink_enable_flg = 5; counter_skip_flg = 0;
        }
        else {
            BLINK_TIME = 300;
            if( SYSTEM_STATE_FLG == 203 ) blink_enable_flg = 0;
        }
        ...
        switch( blink_enable_flg ) {
            ...
            case 5: BlinkDisable(); blink_pos[ 0 ] = 1; blink_pos[ 1 ] = 1; blink_pos[ 2 ] = 1; blink_pos[ 3 ] = 1; blink_pos[ 4 ] = 1; blink_pos[ 5 ] = 1; blink_pos[ 6 ] = 1; break;
        }


        switch( SYSTEM_STATE_FLG ) {
            case 100: UpdateDisplay( Ds1307HMSRead(), 1, 6 ); blink_enable_flg = 0; break;
        }
...
void LEDProcess() {
    ...
    switch( SYSTEM_STATE_FLG ){
        ...
        case 101:
            ...
            return;
        case 200:
            if(counter_down_comp_flg) { display_datas[ 7 ] |= 0x10; }
            else display_datas[ 7 ] |= 0x20;
            if( counter_skip_flg ) {
                led_blink_cnt ++;
                if( led_blink_cnt < 25 / 2 ) display_datas[ 7 ] |= 0x04;
                if( led_blink_cnt > 25 ) led_blink_cnt = 0;
            }
            break;
        case 201:
...
void DetectStatus(){
    ...
    else if( PORTAbits.RA0 == 0 && PORTAbits.RA1 == 1 && PORTAbits.RA2 == 1 && PORTAbits.RA5 == 1 ) {
        SYSTEM_STATE_FLG = 100; blink_enable_flg = 0;
    }
}
...
void InitValue(){
    if( SYSTEM_STATE_FLG == 100 ){
        ...
        if( real_rtc_data[ 2 ] == 0 && real_rtc_data[ 1 ] == 0 && real_rtc_data[ 0 ] == 0 ) Ds1307Write( &rtc_default_values );
    }
    else if( SYSTEM_STATE_FLG == 201 ){
