...
void main(void) {
    ...
    while(1)
    {
        ...
        if( SYSTEM_STATE_FLG == 101 && set_mode_release_cnt > 400 ) {
            Ds1307Write( &real_rtc_data ); set_mode_release_cnt = 0;
            SYSTEM_STATE_FLG = 100;
        }

        if( counter_down_comp_flg ) {
            BLINK_TIME = 80; blink_enable_flg = 5;
        }
        else { BLINK_TIME = 300; if( SYSTEM_STATE_FLG == 203 ) blink_enable_flg = 0; }
        ...
        switch( blink_enable_flg ) {
            ...
            case 1: BlinkDisable(); blink_pos[ 6 ] = 1; break;
            case 2: BlinkDisable(); blink_pos[ 0 ] = 1; blink_pos[ 1 ] = 1; break;
            case 3: BlinkDisable(); blink_pos[ 2 ] = 1; blink_pos[ 3 ] = 1; break;
            case 4: BlinkDisable(); blink_pos[ 4 ] = 1; blink_pos[ 5 ] = 1; break;
            case 5: BlinkDisable(); blink_pos[ 0 ] = 1; blink_pos[ 1 ] = 1; blink_pos[ 2 ] = 1; blink_pos[ 3 ] = 1; blink_pos[ 4 ] = 1; blink_pos[ 5 ] = 1; break;
        }


        switch( SYSTEM_STATE_FLG ) {
            case 100: UpdateDisplay( Ds1307Read( rtc_clock_disp_mode ), 1, 6 ); break;
            case 101: ClockSetting(); break;
        }


        if( counter_skip_flg ) continue;
        switch( SYSTEM_STATE_FLG ) {
            case 200: UpdateDisplay( ReadCounter(), 1, 6 ); break;
            case 201: UpdateDisplay( ReadCounter(), 1, 6 ); break;
            case 202: UpdateDisplay( ReadCounter(), 1, 6 ); break;
            case 203: UpdateDisplay( ReadCounter(), 1, 6 ); break;
        }
    }
}
...
void DeviceInit(){
    ...
    ADCON0 = 0x00; ADCON1 = 0x0F;
    EnableAllInterrupts();


    SYSTEM_STATE_FLG = 201;

    OpenTimer0( 0b11111111 & 0b10111111 & 0b11011111 & 0b11110010 );
    WriteTimer0( 65285 ); StartTimer0();
...
void ClockSetting() {
    display_datas[0] = MarkDisplay( real_rtc_data[ 2 ] >> 4 );
    display_datas[1] = MarkDisplay( real_rtc_data[ 2 ] % 16 );
    display_datas[2] = MarkDisplay( real_rtc_data[ 1 ] >> 4 );
    display_datas[3] = MarkDisplay( real_rtc_data[ 1 ] % 16 );
    display_datas[4] = MarkDisplay( real_rtc_data[ 0 ] >> 4 );
    display_datas[5] = MarkDisplay( real_rtc_data[ 0 ] % 16 );
}

void LEDProcess() {
    display_datas[ 7 ] &= 0x00;
    switch( SYSTEM_STATE_FLG ){
        case 100:
        case 101:
            if( rtc_clock_disp_type == 0 ) display_datas[ 7 ] |= 0x01;
            else display_datas[ 7 ] |= 0x02;
            return;
        case 200:
            display_datas[ 7 ] |= 0x20;
            if( counter_skip_flg ) {
                led_blink_cnt ++;
                if( led_blink_cnt < 40 / 2 ) display_datas[ 7 ] |= 0x04;
                if( led_blink_cnt > 40 ) led_blink_cnt = 0;
            }
            break;
        case 201:
            display_datas[ 7 ] |= 0x10;
            break;
        case 202:
        case 203:
            break;
    }
    if( counter_direction_flg == 1 ) { display_datas[ 7 ] |= 0x01; }
    else { display_datas[ 7 ] |= 0x02; }
}
...
void DetectStatus(){
    ...
    TRISAbits.TRISA2 = 1;
    TRISAbits.TRISA5 = 1;
    if( PORTAbits.RA0 == 1 && PORTAbits.RA1 == 1 && PORTAbits.RA2 == 1 && PORTAbits.RA5 == 1 ) {
        SYSTEM_STATE_FLG = 201; blink_enable_flg = 0;
    }
    else if( PORTAbits.RA0 == 0 && PORTAbits.RA1 == 0 && PORTAbits.RA2 == 0 && PORTAbits.RA5 == 0 ) {
        SYSTEM_STATE_FLG = 100; blink_enable_flg = 1;
    }
}

void InitValue(){
    if( SYSTEM_STATE_FLG == 100 ){
# 173 "Main.c"
    }
    else if( SYSTEM_STATE_FLG == 200 ){
        counter_enable_flg = 0; counter_skip_flg = 0;
    }
}
